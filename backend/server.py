...
SENSOR_STATE: Dict[str, Dict] = {
    "A1": {"id":"A1","crop":"Jagung", "vwc":38.0,"temp":34.2,"batt":91,"thr":35,"dose":80, "loc":"Blok Utara",   "node":"ESP-001","status":"warn"},
    "A2": {"id":"A2","crop":"Kedelai","vwc":52.3,"temp":32.1,"batt":78,"thr":42,"dose":85, "loc":"Blok Utara",   "node":"ESP-002","status":"ok"},
    "B1": {"id":"B1","crop":"Cabai",  "vwc":28.1,"temp":33.8,"batt":65,"thr":38,"dose":120,"loc":"Blok Selatan", "node":"LoRa-001","status":"crit"},
    "B2": {"id":"B2","crop":"Tomat",  "vwc":61.0,"temp":31.5,"batt":34,"thr":40,"dose":70, "loc":"Blok Selatan", "node":"LoRa-002","status":"ok"},
}
...
async def sensor_simulator():
    ...
    while True:
        ...
        for fid, f in SENSOR_STATE.items():
            ...
            f["vwc"] = round(max(15.0, min(85.0, f["vwc"] + drift + (random.random() - 0.54) * 0.3)), 2)
        ...
        msg = json.dumps({
            "type":    "sensor_batch",
            "fields":  {fid: {"vwc": f["vwc"], "temp": f["temp"], "status": f["status"]} for fid, f in SENSOR_STATE.items()},
            "ts":      ts,
            "tick":    tick,
        })
        dead = set()
...
async def do_irrigate(field_id: str, dose_g: int, trigger: str = "manual"):
    ...
    HYDROGEL_STOCK["history"].append({
        "ts": datetime.now(timezone.utc).isoformat(),
        "field_id": field_id, "delta_g": -dose_g,
    ...
    SENSOR_STATE[field_id]["vwc"] = min(85.0, SENSOR_STATE[field_id]["vwc"] + dose_g * 0.06)
...
async def serve_frontend(path: str = ""):
    ...
    return HTMLResponse("<h3>Frontend not found — letakkan index.html di folder frontend/</h3>", 404)
...
async def sensors_snapshot():
    return {
        "snapshot":   {fid: {"vwc": f["vwc"], "temp": f["temp"], "status": f["status"]}
                       for fid, f in SENSOR_STATE.items()},
        "fetched_at": datetime.now(timezone.utc).isoformat(),
    }
...
async def trigger_irrigation(cmd: IrrigateCmd):
    ...
    if HYDROGEL_STOCK["stock_g"] < cmd.dose_g:
        raise HTTPException(400, f"Stok hidrogel tidak cukup ({HYDROGEL_STOCK['stock_g']}g tersisa)")
            ...
            try:
                ...
                if data.get("type") == "ping":
                    ...
                    if fid in SENSOR_STATE:
                        ...
                        await ws.send_text(json.dumps({"type": "irrigate_ok", "field_id": fid, "dose_g": dose}))
